...
- **通道寬度全部統一成第三關的 ±4.7。**原作第二關「排水溝」只有 ±2.2，
  在這個鏡頭距離下根本閃不掉。原始值仍記錄在上面的對照表裡。
- 音樂改用 **MP3**。原始檔是 Ogg Vorbis，但 Safari 不支援，
  iPhone / iPad 上會整個沒聲音。原始 OGG 保存在私有封存庫。

### 現在用的是當年的 3D 模型

`.msh` 也解開了。格式：檔頭帶 bounding sphere 與 bbox，之後每個子網格是
`0xffffffff` 標記 + 頂點數 + stride，頂點為 **position(3f) + normal(3f) + uv(2f)
共 32 bytes**，再接索引數與 u16 索引。30 個全部解出，零失敗。

遊戲現在直接載入這些幾何（`assets/models.json`）：

| 物件 | 原始模型 | 頂點 / 三角形 |
|---|---|---|
| 當奇 | `move_Sphere01-lib` + `move_Object01-lib` | 537 / 484 |
| 問號方塊 | `Item01_Object01-lib` | 168 / 84 |
| 鯊魚 | `shark_Sphere01-lib` | 297 / 144 |
| 可樂罐障礙 | `Barriers_01_Cylinder01-lib` | 34 / 28 |
| 石頭 | `rock_Sphere01-lib` | 90 / 116 |
| 瓶子 | `Bottles_Gengon01-lib` | 256 / 156 |

### 場景重複的機制

`Action_Ai_Handler_onRoom1_move` 說得很清楚 —— 場景由**兩塊輪流搬動**：

```
Plane / Plane2                ← 兩塊場景
random_place / random_place2  ← 兩個障礙物生成區
Plane.z -= 76                 ← 每塊長 76 單位
Room1_DetectZ -= 38           ← 兩塊交替，所以觸發間隔是 38
sendEvent(Time_Ai, onScorecount)  ← 計分檢查點就在這裡
```

所以計分檢查點的間距是 **38**。把 46.6 寬的牆板縮到 38 長，換算縮放是
0.815 —— 而 `.scn` 裡記錄的實際縮放是 **0.85**，兩者幾乎吻合，等於互相驗證。

遊戲現在用真正的牆板幾何（`scene10929_scene1-lib` 等三塊，載入自
`assets/scenes.json`），沿著同樣的 38 單位節奏重複，障礙物依舊隨機 ——
跟原作一樣。

### 沒能照搬的部分

- `.scn` 的物件記錄格式沒有完全反解。變換本身可以用特徵掃描抓出來
  （position 3f + 單位四元數 4f + scale 3f，共 40 bytes，見
  `tools/scn_objects.py`），但物件與網格的對應關係還不可靠
- **體積上限**：原始碼只有下限沒有上限，這裡加了 2.2 的上限，否則會撐爆畫面
- **左右移動速度**：沒能從常數中確定，是自己調的

## 載入

進遊戲前會先把**全部資產抓完**：`models.json`、`scenes.json`、21 張貼圖、
五首配樂，載入畫面用當年的 THYY logo 配一條進度條。音樂用 `canplaythrough`
判斷就緒，但有些瀏覽器要等使用者互動才會發，所以每首另外給四秒的退場，
整體再加一道 12 秒的保險，不會把人卡在進度條後面。
